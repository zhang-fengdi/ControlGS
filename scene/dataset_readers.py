...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder, test_cam_names_list, masks_folder):
    cam_infos = []
    mask_count = 0
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params,
                              ...
                              width=width, height=height, is_test=image_name in test_cam_names_list, image=image, mask=mask)
...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, llffhold=8, masks=None):
    ...
    cam_infos_unsorted = readColmapCameras(
        ...
        depths_folder=os.path.join(path, depths) if depths != "" else "", test_cam_names_list=test_cam_names_list, masks_folder=masks)
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX,
                            image_path=image_path, mask_path=None, image_name=image_name,
                            width=image.size[0], height=image.size[1], depth_path=depth_path, depth_params=None, is_test=is_test, image=image, mask=None))
