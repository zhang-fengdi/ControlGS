...
class GaussianModel:
    ...
    @property
    def get_min_split_count(self):
        return self._min_split_count
    
    @property
    def get_densification_level(self):
        return self.densification_level
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_split_count):
        d = {"xyz": new_xyz,
    ...
    def get_expon_lr(self, lr_init, lr_final, start_step, max_steps, step):
        if step < start_step:
            return lr_init
        if step > max_steps:
            return lr_final
        t = np.clip((step - start_step) / (max_steps - start_step), 0, 1)
        return np.exp(np.log(lr_init) * (1 - t) + np.log(lr_final) * t)
        
    def prune_by_opacity(self, min_opacity, extent=None):
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if extent is not None:
            big_points_ws = self.get_scaling.max(dim=1).values > 1.0 * extent
    ...
    def octree_densify(self, batchsize):
        ...
        positions_batch      = self._xyz.detach()[batch_indices]
        features_dc_batch    = self._features_dc.detach()[batch_indices]
        features_rest_batch  = self._features_rest.detach()[batch_indices]
        rotation_raw_batch   = self._rotation.detach()[batch_indices]
        scaling_raw_batch    = self.get_scaling.detach()[batch_indices]
        opacity_raw_batch    = self.get_opacity.detach()[batch_indices]
        split_count_batch    = self._split_count[batch_indices]
        ...
        N = self._xyz.shape[0]
        prune_mask = torch.zeros(N, dtype=bool, device=self._xyz.device)
